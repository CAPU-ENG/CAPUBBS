...
each unique URL, and stores a verified single-frame image no larger than 1 MiB.
"""
...
def compress_image(
    ...
    source_frame_count = source_info["frameCount"]
    extension = FORMAT_EXTENSIONS.get(source_format)
    if len(data) <= max_bytes and extension is not None and source_frame_count == 1:
        return data, {
    ...
    scale = 1.0
    smallest_candidate: tuple[bytes, str, tuple[int, int], int] | None = None
    ...
    try:
        while True:
            ...
            for quality in (88, 78, 68, 58, 48, 38):
                try:
                    candidate, output_format = encode_candidate(
                        source, size, quality, alpha
                    )
