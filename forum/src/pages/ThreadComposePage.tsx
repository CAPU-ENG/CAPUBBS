...
export function ThreadComposePage() {
  const request = useMemo(getComposeRequest, []);
  ...
  useEffect(() => {
    if (!request || !board || authStatus !== 'authenticated' || !ownerKey) return;
    let active = true;

    void readStoredThreadComposeDraft(request.bid, ownerKey).then((draft) => {
    ...
    return () => { active = false; };
  }, [authStatus, board, ownerKey, request]);
  ...
  function leaveEditor() {
    if (!isDirty || window.confirm('放弃尚未保存的发帖内容？')) {
  ...
  async function saveDraft() {
    if (!request || !board || !ownerKey || isSavingDraft) return;
    if (!title.trim() && !contentReady && attachments.length === 0) {
      setStatus('没有可保存的发帖内容');
      setStatusIsError(true);
      return;
    }
    ...
    try {
      await saveStoredThreadComposeDraft({
        ...
        excerpt: getDraftExcerpt(editorValue, attachments),
        signatureIndex,
  ...
  async function publish() {
    ...
    try {
      ...
      if (ownerKey) {
        try {
          await deleteStoredThreadComposeDraft(request.bid, ownerKey);
        } catch {
          // The thread is already published; stale local draft cleanup must not invite a duplicate post.
        }
      }
      window.location.href = published.tid
        ? `/?bid=${published.bid}&p=1&tid=${published.tid}#1`
        : `/?bid=${published.bid}`;
    } catch (error) {
      setStatus(error instanceof Error ? error.message : '主题发表失败，请稍后重试。');
      setStatusIsError(true);
  ...
  const pagePending = Boolean(request && !loadError && (!board || authPending));

  return (
    <div className="relative min-h-screen text-[var(--text)] transition-colors duration-200">
      <AppBackground />
      <TopBar contextHref="#compose-page-title" contextTitle={board?.name} />
      <main className="thread-edit-page-shell thread-compose-page-shell">
        {!request ? (
          ...
          <ComposeRequestState backHref={boardHref} description={loadError} title="暂时无法进入发帖页" />
        ) : authStatus === 'guest' ? (
          <ComposeRequestState
            backHref={boardHref}
            description="登录后才能发表新主题，当前内容不会被提交。"
          ...
          <>
            <header className="thread-edit-heading-card">
              <button aria-label="返回版面" className="thread-edit-back" onClick={leaveEditor} type="button">
                <ArrowLeft size={19} />
              </button>
              <div className="thread-edit-heading-copy">
                <span>{board.name} / 发帖</span>
                <h1 id="compose-page-title">发表新主题</h1>
              </div>
            </header>

            <section className="thread-edit-source-card thread-compose-context-card" aria-label="发帖信息">
              <div><span>目标版块</span><strong>{board.name}</strong></div>
              <div><span>发布账号</span><strong>{viewer?.username ?? '—'}</strong></div>
              <div><span>保存状态</span><strong>{isDirty ? '有未保存内容' : '内容已保存'}</strong></div>
              <div><span>附件</span><strong>{attachments.length ? `${attachments.length} 个` : '暂无'}</strong></div>
            </section>

            <PostEditor
              ariaLabel={`在「${board.name}」发表新主题`}
              attachmentDialogDescription="文件会立即上传，并在发表主题后关联到首楼"
              attachmentLabel="主题附件"
              ...
              heading="新主题"
              headingMeta={title.trim() ? title.trim() : `发布到 ${board.name}`}
              name="thread-compose-signature"
              ...
              placeholder="写下正文，可以补充背景、细节和你希望大家讨论的问题……"
...
function getComposeRequest() {
  const value = Number(new URLSearchParams(window.location.search).get('bid'));
  return Number.isSafeInteger(value) && value > 0 ? { bid: value } : null;
}
...
function getDraftExcerpt(editor: RichTextEditorValue, attachments: ThreadAttachmentInfo[]) {
  const text = editor.mode === 'markdown'
    ...
    : editor.content.replace(/<br\s*\/?>/gi, ' ')
      ...
      .replace(/\s+/g, ' ')
      .trim();
  return text || (attachments.length ? '附件发帖草稿' : '空白发帖草稿');
}
...
async function publishThread({
  ...
  if (!response.ok || payload.code !== 0) {
    throw new ThreadApiError(payload.message?.trim() || '主题发表失败，请稍后重试。');
  }
