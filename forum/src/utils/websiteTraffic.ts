...
export function isTrafficDateInPeriod(date: string, endDate: string, period: TrafficPeriod): boolean {
  const timestamp = activityDate(date);
  const end = activityDate(endDate);
  if (timestamp === null || end === null || timestamp > end) return false;
  const days = TRAFFIC_PERIODS.find((item) => item.id === period)!.days;
  return end - timestamp < days * 86400000;
}
