...
export function YahouLineageOverview({ data, onClose }: { data: YahouLineage; onClose: () => void }) {
  ...
  const result = useMemo(() => {
    try { return { graph: buildYahouGraph(data), error: '' }; }
    catch { return { graph: null, error: '谱系总览生成失败，请关闭后重试。' }; }
  }, [data]);
  ...
  return createPortal(
    <DialogNativeLayer aria-labelledby={titleId} className="yahou-workspace yahou-overview-dialog"
      ...
      {result.graph ? <RGProvider><OverviewNetwork graph={result.graph} /></RGProvider> : <p className="yahou-load-state" role="alert">{result.error}</p>}
