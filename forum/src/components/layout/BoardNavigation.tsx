...
const primaryBoards: Board[] = [
  ...
  { id: 4, label: '纯净水', description: '轻松闲聊与校园日常', icon: Flame },
  ...
  { id: 28, label: '网站维护', description: '站务公告与问题反馈', icon: QrCode },
];

// 当前公开版块只有以上九个；新板块接入后放入此层，不改变主要版块排序。
...
export function DesktopBoardDrawer({ onNavigate }: { onNavigate: () => void }) {
  return (
    <section className="board-drawer" aria-label="版块导航">
      <div className="board-drawer-intro">
        <p className="eyebrow">BOARD DIRECTORY</p>
        <div className="mt-1 flex items-end justify-between gap-6">
          <div>
            <h2 className="section-title text-2xl">选择版块</h2>
            <p className="mt-1 text-sm text-[var(--text-muted)]">从一段骑行、一项技术或一场讨论开始。</p>
          </div>
          <span className="hidden text-xs text-[var(--text-faint)] sm:block">9 个主要版块</span>
        </div>
      </div>
      ...
      <div className="board-drawer-section border-t border-[var(--line)]">
        <p className="drawer-section-label">其他板块</p>
        {otherBoards.length > 0 ? (
          <div className="mt-3 flex flex-wrap gap-2">
            {otherBoards.map(({ id, label }) => (
              <a className="board-chip" href={boardHref(id)} key={id} onClick={onNavigate}>{label}</a>
            ))}
          </div>
        ) : (
          <p className="mt-2 text-sm text-[var(--text-faint)]">暂无其他公开板块</p>
        )}
      </div>

      <div className="board-drawer-section flex items-center gap-3 border-t border-[var(--line)]">
        <span className="drawer-section-label mr-2">补充内容</span>
  ...
  return (
    ...
    >
      ...
      <div className="mobile-sidebar-scroll">
        ...
        <div className="mt-6">
          <p className="drawer-section-label px-1">主要版块</p>
        ...
        <div className="mt-6 border-t border-[var(--line)] pt-5">
          <p className="drawer-section-label px-1">其他板块</p>
          {otherBoards.length > 0 ? (
            <div className="mt-2 space-y-1">
              {otherBoards.map(({ id, label, icon: Icon }) => (
                <a className="mobile-list-link" href={boardHref(id)} key={id} onClick={onClose}>
                  <Icon size={16} /> {label}
                </a>
              ))}
            </div>
          ) : (
            <p className="px-1 pt-2 text-sm text-[var(--text-faint)]">暂无其他公开板块</p>
          )}
        </div>

        <div className="mt-6 border-t border-[var(--line)] pt-5">
          <p className="drawer-section-label px-1">补充内容</p>
          <a className="mobile-list-link mt-2" href="/index/data.php" onClick={onClose}>
