import{p as ze,r as s,j as e,Z as _e,X as ge,b as qe,d as Ge,aM as Ue,aN as Be,aO as Ke,az as Ye,ao as We,aP as Xe,aQ as Je,aR as Ve,aS as Qe,F as se,w as Ze,J as et,aK as tt}from"./index-BI7nhiCU.js";import{e as be,m as le,s as rt,f as at,r as nt,h as st,b as me,a as we,P as je}from"./RichTextEditor.gallery-8qWpjeEt.js";import{P as it}from"./plus-Bhn34cBB.js";import{R as ot}from"./rotate-ccw-BRB-RPDY.js";import{l as lt}from"./thread-C5ItfEHh.js";import{a as ct,r as dt,t as ut}from"./forumMarkup-CECtPv9S.js";import{D as Re,T as ce}from"./TagBadge-DL_rGzoB.js";import{T as fe}from"./trash-2-B7fElkUO.js";import{P as xe}from"./pencil-DVKkNEAn.js";import{E as gt}from"./external-link-DBU2vDvP.js";import{T as mt}from"./triangle-alert-D1kKXZMg.js";const ft=[["path",{d:"M20 18v-2a4 4 0 0 0-4-4H4",key:"5vmcpk"}],["path",{d:"m9 17-5-5 5-5",key:"nvlc11"}]],pt=ze("reply",ft);async function ht(t){try{if(navigator.clipboard?.writeText)return await navigator.clipboard.writeText(t),!0}catch{}const r=document.createElement("textarea");r.value=t,r.setAttribute("readonly",""),r.style.position="fixed",r.style.opacity="0",document.body.appendChild(r),r.select();try{return document.execCommand("copy")}finally{r.remove()}}function Ee({className:t="",html:r,onImageOpen:n,variant:f}){const a=s.useRef(null),i=s.useMemo(()=>({__html:r}),[r]);if(s.useEffect(()=>{const o=a.current;if(!o)return;const u=Array.from(o.querySelectorAll("img")),y=h=>{h.dataset.capubbsImageLoaded="true"},w=u.map(h=>{if(h.complete)return y(h),null;const j=()=>y(h);return h.addEventListener("load",j,{once:!0}),h.addEventListener("error",j,{once:!0}),{handleLoad:j,image:h}});return()=>{w.forEach(h=>{h&&(h.image.removeEventListener("load",h.handleLoad),h.image.removeEventListener("error",h.handleLoad))})}},[r]),!r)return null;function m(o,u){if(!n||!(o instanceof Element))return;const y=o.closest("img");if(!(y instanceof HTMLImageElement))return;const w=Array.from(u.querySelectorAll("img")),h=w.indexOf(y);if(h<0)return;const j=w.map(R=>yt(R,u)),C=w.map((R,v)=>{const S=j[v];return{alt:R.alt.trim(),src:R.currentSrc||R.src,...S?{galleryId:S.galleryId,galleryIndex:S.galleryIndex}:{}}});n(C,h,y,R=>{const v=j[R];v&&rt(v.gallery,v.galleryIndex)})}function l(o){const u=be(o.target);if(u&&o.target instanceof Element){o.preventDefault(),o.stopPropagation(),le(o.target,u);return}!n||!(o.target instanceof HTMLImageElement)||(o.preventDefault(),m(o.target,o.currentTarget))}function p(o){const u=be(o.target);if(u&&["Enter"," "].includes(o.key)&&o.target instanceof Element){o.preventDefault(),le(o.target,u);return}if(["ArrowLeft","ArrowRight"].includes(o.key)&&o.target instanceof Element&&o.target.closest(".capubbs-gallery")){o.preventDefault(),le(o.target,o.key==="ArrowLeft"?"prev":"next");return}!n||!(o.target instanceof HTMLImageElement)||!["Enter"," "].includes(o.key)||(o.preventDefault(),m(o.target,o.currentTarget))}return e.jsx("div",{ref:a,className:`forum-markup forum-markup-${f} ${t}`.trim(),"data-forum-markup":f,dangerouslySetInnerHTML:i,onClick:l,onKeyDown:p})}function yt(t,r){const n=t.closest(".capubbs-gallery");if(!n||!r.contains(n))return null;const a=Array.from(r.querySelectorAll(".capubbs-gallery")).indexOf(n),m=Array.from(n.querySelectorAll('[data-capubbs-gallery-slide="true"] img')).indexOf(t);return a>=0&&m>=0?{gallery:n,galleryId:a,galleryIndex:m}:null}const $=1,Se=4,ie=.25;function bt(t){return Math.min(Se,Math.max($,t))}function de(t){const[r,n]=[...t.values()];return!r||!n?null:Math.hypot(n.x-r.x,n.y-r.y)}function xt({images:t,initialImageIndex:r,onImageChange:n,onClose:f}){const a=Math.min(Math.max(0,r),Math.max(0,t.length-1)),[i,m]=s.useState(a),[l,p]=s.useState($),[o,u]=s.useState({x:0,y:0}),[y,w]=s.useState(!1),h=s.useRef(null),j=s.useRef(null),C=s.useRef(null),M=s.useRef(null),R=s.useRef(a),v=s.useRef($),S=s.useRef({x:0,y:0}),D=s.useRef(null),E=s.useRef(!1),I=s.useRef(new Map),k=s.useRef(null),F=s.useRef($),N=s.useRef(n),Q=s.useRef(f);N.current=n,Q.current=f;function Z(c,b=v.current){const x=h.current,L=C.current;if(!x||!L||b<=$)return{x:0,y:0};const P=Math.max(0,(L.clientWidth*b-x.clientWidth)/2),A=Math.max(0,(L.clientHeight*b-x.clientHeight)/2);return{x:Math.min(P,Math.max(-P,c.x)),y:Math.min(A,Math.max(-A,c.y))}}function B(c,b=v.current){const x=Z(c,b);S.current=x,u(x)}function z(c){const b=Math.round(bt(c)*100)/100;v.current=b,p(b),B(S.current,b)}function G(){v.current=$,S.current={x:0,y:0},p($),u({x:0,y:0})}function Y(c){const b=Math.min(Math.max(0,c),t.length-1);b!==R.current&&(R.current=b,m(b),G(),N.current?.(b))}function X(){Q.current(R.current)}s.useEffect(()=>{const c=document.body.style.overflow,b=document.activeElement,x=h.current;document.body.style.overflow="hidden",M.current?.focus();function L(g){if(g.key==="Escape"){g.preventDefault(),X();return}if(g.key==="ArrowLeft"){g.preventDefault(),g.stopPropagation(),Y(R.current-1);return}if(g.key==="ArrowRight"){g.preventDefault(),g.stopPropagation(),Y(R.current+1);return}if(g.key==="+"||g.key==="="){g.preventDefault(),g.stopPropagation(),z(v.current+ie);return}if(g.key==="-"){g.preventDefault(),g.stopPropagation(),z(v.current-ie);return}if(g.key==="0"){g.preventDefault(),g.stopPropagation(),G();return}if(g.key==="Tab"){const O=j.current?.querySelectorAll("button:not(:disabled)");if(!O?.length)return;const V=O[0],ae=O[O.length-1],ne=document.activeElement;if(g.shiftKey&&ne===V){g.preventDefault(),ae.focus();return}if(!g.shiftKey&&ne===ae){g.preventDefault(),V.focus();return}j.current?.contains(ne)||(g.preventDefault(),V.focus())}}function P(g){if(g.preventDefault(),g.stopPropagation(),g.deltaY===0)return;const O=g.ctrlKey?.01:.002;z(v.current*Math.exp(-g.deltaY*O))}function A(g){g.preventDefault(),g.stopPropagation(),F.current=v.current}function H(g){if(g.preventDefault(),g.stopPropagation(),I.current.size>=2)return;const O=g.scale;typeof O=="number"&&z(F.current*O)}function re(){B(S.current,v.current)}return document.addEventListener("keydown",L,{capture:!0}),window.addEventListener("resize",re),x?.addEventListener("wheel",P,{passive:!1}),x?.addEventListener("gesturestart",A,{passive:!1}),x?.addEventListener("gesturechange",H,{passive:!1}),x?.addEventListener("gestureend",H,{passive:!1}),()=>{document.removeEventListener("keydown",L,{capture:!0}),window.removeEventListener("resize",re),x?.removeEventListener("wheel",P),x?.removeEventListener("gesturestart",A),x?.removeEventListener("gesturechange",H),x?.removeEventListener("gestureend",H),document.body.style.overflow=c,b instanceof HTMLElement&&b.focus()}},[]),s.useEffect(()=>{[t[i-1],t[i+1]].forEach(c=>{if(!c)return;const b=new Image;b.src=c.src})},[i,t]);function ee(c,b,x){D.current={pointerId:c,startX:b,startY:x,originX:S.current.x,originY:S.current.y},w(!0)}function W(c){if(c.target instanceof Element&&c.target.closest("button, .thread-image-lightbox-controls"))return;const b=c.pointerType==="touch",x=c.pointerType==="mouse"&&c.button===0;if(!(!b&&!x)&&(E.current=!1,!(!b&&v.current<=$))){if(c.preventDefault(),c.currentTarget.setPointerCapture(c.pointerId),b&&(I.current.set(c.pointerId,{x:c.clientX,y:c.clientY}),I.current.size===2)){k.current=de(I.current),D.current=null,w(!1);return}v.current>$&&ee(c.pointerId,c.clientX,c.clientY)}}function J(c){const b=I.current.has(c.pointerId),x=D.current;if(!b&&x?.pointerId!==c.pointerId)return;if(c.preventDefault(),c.stopPropagation(),b&&I.current.set(c.pointerId,{x:c.clientX,y:c.clientY}),I.current.size===2){const A=de(I.current),H=k.current;if(!A||!H){k.current=A;return}Math.abs(A-H)>1&&(E.current=!0),z(v.current*(A/H)),k.current=A;return}if(!x||v.current<=$)return;const L=c.clientX-x.startX,P=c.clientY-x.startY;Math.hypot(L,P)>3&&(E.current=!0),B({x:x.originX+L,y:x.originY+P})}function te(c){const b=I.current.delete(c.pointerId),x=D.current?.pointerId===c.pointerId;if(!(!b&&!x)){if(k.current=I.current.size===2?de(I.current):null,I.current.size===1&&v.current>$){const[L]=I.current.entries();if(L){const[P,A]=L;ee(P,A.x,A.y)}}else D.current=null,w(!1);c.currentTarget.hasPointerCapture(c.pointerId)&&c.currentTarget.releasePointerCapture(c.pointerId)}}const U=Math.round(l*100),_=t[i]??t[0];return _?_e.createPortal(e.jsx("div",{className:"thread-image-lightbox-backdrop","data-can-pan":l>$,"data-dragging":y,onClick:c=>{c.target===c.currentTarget&&!E.current&&X()},onPointerCancel:te,onPointerDown:W,onPointerMove:J,onPointerUp:te,ref:h,role:"presentation",children:e.jsxs("figure",{"aria-label":_.alt?`图片预览：${_.alt}（${i+1}/${t.length}）`:`图片预览（${i+1}/${t.length}）`,"aria-modal":"true",className:"thread-image-lightbox",ref:j,role:"dialog",children:[e.jsx("button",{"aria-label":"关闭图片预览",className:"thread-image-lightbox-close",onClick:X,ref:M,type:"button",children:e.jsx(ge,{size:20})}),t.length>1&&e.jsxs(e.Fragment,{children:[e.jsx("button",{"aria-label":"上一张图片",className:"thread-image-lightbox-nav thread-image-lightbox-prev",disabled:i===0,onClick:()=>Y(i-1),title:"上一张（←）",type:"button",children:e.jsx(qe,{size:28})}),e.jsx("button",{"aria-label":"下一张图片",className:"thread-image-lightbox-nav thread-image-lightbox-next",disabled:i===t.length-1,onClick:()=>Y(i+1),title:"下一张（→）",type:"button",children:e.jsx(Ge,{size:28})})]}),e.jsx("img",{alt:_.alt,draggable:"false",onLoad:()=>B(S.current,v.current),ref:C,src:_.src,style:{transform:`translate3d(${o.x}px, ${o.y}px, 0) scale(${l})`}}),_.alt&&e.jsx("figcaption",{children:_.alt}),e.jsxs("div",{"aria-label":"图片缩放",className:"thread-image-lightbox-controls",role:"toolbar",children:[e.jsx("button",{"aria-label":"缩小图片",disabled:l<=$,onClick:()=>z(l-ie),title:"缩小（-）",type:"button",children:e.jsx(at,{size:18})}),e.jsxs("output",{"aria-label":"当前缩放比例","aria-live":"polite",children:[U,"%"]}),e.jsx("button",{"aria-label":"放大图片",disabled:l>=Se,onClick:()=>z(l+ie),title:"放大（+）",type:"button",children:e.jsx(it,{size:18})}),e.jsx("button",{"aria-label":"恢复原始大小",disabled:l===$,onClick:G,title:"恢复原始大小（0）",type:"button",children:e.jsx(ot,{size:17})})]})]})}),document.body):null}const vt="/assets/thread-html-frame-DuCry7dF.css",It=28,wt=64,jt=5e4,Rt=30,Ae=30,K="capubbs-thread-html-frame",Et=new URL(vt,window.location.origin).href;function ve({className:t="",floor:r,html:n,isActivitySignupCanceled:f=!1,onImageOpen:a,variant:i}){const m=s.useMemo(()=>i==="signature"?nt(n):n,[n,i]),l=At(m,i==="signature"),p=ct(l),o=s.useMemo(()=>p?null:dt(l,{normalizeLegacyLineBreaks:i==="signature"}),[l,p,i]),u=s.useMemo(()=>ut(l),[l]);return!p&&o!==null?e.jsx(Ee,{className:t,html:o,onImageOpen:a,variant:i}):e.jsx(St,{className:t,floor:r,html:u,isActivitySignupCanceled:f,onImageOpen:a,variant:i})}function St({className:t,floor:r,html:n,isActivitySignupCanceled:f,onImageOpen:a,variant:i}){const m=s.useRef(null),l=s.useRef(`${i}-${r}-${Math.random().toString(36).slice(2)}`),p=s.useRef(a);p.current=a;const o=i==="signature"?It:wt,u=!!a,[y,w]=s.useState(null),h=$t(),j=s.useRef(h),C=Ue(),M=i==="signature"?14:C,R=s.useMemo(()=>Tt({canOpenImages:u,frameId:l.current,html:n,isActivitySignupCanceled:f,isDarkTheme:j.current,fontSize:M,variant:i}),[u,M,n,f,i]),v=s.useMemo(()=>`data:text/html;charset=utf-8,${encodeURIComponent(R)}`,[R]),S=s.useCallback(()=>{m.current?.contentWindow?.postMessage({frameId:l.current,source:K,theme:h?"dark":"light",type:"theme"},"*")},[h]);return s.useEffect(()=>{w(null)},[v]),s.useEffect(()=>{S()},[S]),s.useLayoutEffect(()=>{function D(E){if(!(E.source!==m.current?.contentWindow||!Ct(E.data))&&E.data.frameId===l.current){if(E.data.type==="anchor"){const I=m.current;if(!I)return;const k=window.getComputedStyle(document.documentElement),F=Number.parseFloat(k.getPropertyValue("--topbar-height"))||0,N=window.scrollY+I.getBoundingClientRect().top;window.scrollTo({left:0,top:Math.max(0,N+E.data.offsetTop-F-16)});return}if(E.data.type==="navigate"){const I=Be(E.data.url,Te());if(!I)return;window.history.pushState(null,"",I),window.dispatchEvent(new Event(Ke));const k=new URL(I,window.location.origin);k.hash?window.requestAnimationFrame(()=>{const F=decodeURIComponent(k.hash.slice(1)),N=Ye(`#${F}`);(N?We(N):document.getElementById(F))?.scrollIntoView({block:"start"})}):window.scrollTo({left:0,top:0});return}if(E.data.type==="image-open"){const I=m.current;if(!I)return;const k=F=>{const N=E.data.images[F];!N||typeof N.galleryId!="number"||!Number.isSafeInteger(N.galleryIndex)||I.contentWindow?.postMessage({frameId:l.current,galleryId:N.galleryId,galleryIndex:N.galleryIndex,source:K,type:"gallery-select"},"*")};p.current?.(E.data.images,E.data.imageIndex,I,k);return}w(Math.min(jt,Math.max(o,Math.ceil(E.data.height))))}}return window.addEventListener("message",D),()=>window.removeEventListener("message",D)},[o]),e.jsx("iframe",{ref:m,className:`thread-html-frame thread-html-frame-${i} ${t}`.trim(),referrerPolicy:"no-referrer",sandbox:"allow-scripts allow-same-origin",scrolling:"no",src:v,onLoad:S,style:{"--thread-html-frame-width-allowance":`${Ae}px`,...y===null?{}:{"--thread-html-frame-height":`${y}px`}},title:i==="signature"?`第 ${r} 楼签名档`:`第 ${r} 楼正文`})}function At(t,r){const[n,f]=s.useState(t);return s.useEffect(()=>{const a=new AbortController,i=r?st(t):[];if(f(t),i.length===0)return()=>a.abort();const m=Array.from(new Map(i.map(l=>[`${l.bid}:${l.tid}:${l.pid}`,l])).values());return Promise.all(m.map(async l=>{try{const p=await lt(l,a.signal);return[`${l.bid}:${l.tid}:${l.pid}`,p]}catch(p){if(p instanceof DOMException&&p.name==="AbortError")throw p;return[`${l.bid}:${l.tid}:${l.pid}`,""]}})).then(l=>{if(a.signal.aborted)return;const p=new Map(l);let o=t;i.forEach(u=>{const y=p.get(`${u.bid}:${u.tid}:${u.pid}`);y&&(o=o.replace(u.marker,y))}),f(o)}).catch(()=>{}),()=>a.abort()},[r,t]),n}function Tt({canOpenImages:t,frameId:r,fontSize:n,html:f,isActivitySignupCanceled:a,isDarkTheme:i,variant:m}){const l=m==="signature",p=l?"#999999":"rgb(63 63 70)",o=l?"#999999":"rgb(228 228 231)",u=l?"monospace":"'Noto Sans CJK SC','Source Han Sans SC','PingFang SC','Microsoft YaHei',sans-serif",y=l?"padding-top:10px;color:inherit;font-family:inherit;font-size:inherit;":"",w=a?" capubbs-activity-signup-canceled":"";return`<!doctype html>
...
<head>
  ...
  <link rel="stylesheet" href="${Ie(Et)}">
  <style>
