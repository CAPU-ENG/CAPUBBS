import{c as Re,r as i,j as e,ay as Ke,X as re,p as We,d as Xe,Q as Ve,az as Je,aA as Qe,ap as Ze,ak as et,aB as tt,aC as rt,aD as at,aE as nt,i as ie,ad as st,aj as it,V as lt,Y as ot}from"./index-BOlsU4HP.js";import{c as ve,m as de,s as ct,M as dt,r as ut,f as mt,d as pe,e as Ne,P as Se,h as gt,R as ft,b as pt,a as ht,C as je}from"./RichTextEditor-ZFCUCY35.js";import{u as yt}from"./useAuthorProfile-BPxJS9Gl.js";import{P as xt}from"./plus-BXL1amQR.js";import{R as bt}from"./rotate-ccw-t6Cr-Y1F.js";import{m as vt}from"./thread-DfhejnNv.js";import{r as jt,b as It,t as wt}from"./forumMarkup-D9Jy_rxp.js";import{D as Ae,a as ue}from"./TagBadge-DUFM2PKP.js";import{T as oe}from"./trash-2-DuF6ASiB.js";import{C as Et}from"./check-itrXLmdz.js";import{Q as Rt}from"./quote-B_tsUKKU.js";import{P as Ie}from"./pencil-CKVMmk9S.js";import{E as Nt}from"./external-link-BE_pQHam.js";import{T as St}from"./triangle-alert-DTsIfUs1.js";const At=[["path",{d:"m16 6-8.414 8.586a2 2 0 0 0 2.829 2.829l8.414-8.586a4 4 0 1 0-5.657-5.657l-8.379 8.551a6 6 0 1 0 8.485 8.485l8.379-8.551",key:"1miecu"}]],ge=Re("paperclip",At);const kt=[["path",{d:"M20 18v-2a4 4 0 0 0-4-4H4",key:"5vmcpk"}],["path",{d:"m9 17-5-5 5-5",key:"nvlc11"}]],Tt=Re("reply",kt);async function Ct(t){try{if(navigator.clipboard?.writeText)return await navigator.clipboard.writeText(t),!0}catch{}const r=document.createElement("textarea");r.value=t,r.setAttribute("readonly",""),r.style.position="fixed",r.style.opacity="0",document.body.appendChild(r),r.select();try{return document.execCommand("copy")}finally{r.remove()}}function ke({className:t="",html:r,onImageOpen:n,variant:g}){const a=i.useRef(null),l=i.useMemo(()=>({__html:r}),[r]);if(i.useEffect(()=>{const s=a.current;if(!s)return;const m=Array.from(s.querySelectorAll("img")),h=p=>{p.dataset.capubbsImageLoaded="true"},j=m.map(p=>{if(p.complete)return h(p),null;const I=()=>h(p);return p.addEventListener("load",I,{once:!0}),p.addEventListener("error",I,{once:!0}),{handleLoad:I,image:p}});return()=>{j.forEach(p=>{p&&(p.image.removeEventListener("load",p.handleLoad),p.image.removeEventListener("error",p.handleLoad))})}},[r]),!r)return null;function u(s,m){if(!n||!(s instanceof Element))return;const h=s.closest("img");if(!(h instanceof HTMLImageElement))return;const j=Array.from(m.querySelectorAll("img")),p=j.indexOf(h);if(p<0)return;const I=j.map(E=>$t(E,m)),T=j.map((E,x)=>{const S=I[x];return{alt:E.alt.trim(),src:E.currentSrc||E.src,...S?{galleryId:S.galleryId,galleryIndex:S.galleryIndex}:{}}});n(T,p,h,E=>{const x=I[E];x&&ct(x.gallery,x.galleryIndex)})}function o(s){const m=ve(s.target);if(m&&s.target instanceof Element){s.preventDefault(),s.stopPropagation(),de(s.target,m);return}!n||!(s.target instanceof HTMLImageElement)||(s.preventDefault(),u(s.target,s.currentTarget))}function f(s){const m=ve(s.target);if(m&&["Enter"," "].includes(s.key)&&s.target instanceof Element){s.preventDefault(),de(s.target,m);return}if(["ArrowLeft","ArrowRight"].includes(s.key)&&s.target instanceof Element&&s.target.closest(".capubbs-gallery")){s.preventDefault(),de(s.target,s.key==="ArrowLeft"?"prev":"next");return}!n||!(s.target instanceof HTMLImageElement)||!["Enter"," "].includes(s.key)||(s.preventDefault(),u(s.target,s.currentTarget))}return e.jsx("div",{ref:a,className:`forum-markup forum-markup-${g} ${t}`.trim(),"data-forum-markup":g,dangerouslySetInnerHTML:l,onClick:o,onKeyDown:f})}function $t(t,r){const n=t.closest(".capubbs-gallery");if(!n||!r.contains(n))return null;const a=Array.from(r.querySelectorAll(".capubbs-gallery")).indexOf(n),u=Array.from(n.querySelectorAll('[data-capubbs-gallery-slide="true"] img')).indexOf(t);return a>=0&&u>=0?{gallery:n,galleryId:a,galleryIndex:u}:null}const F=1,Te=4,le=.25;function Lt(t){return Math.min(Te,Math.max(F,t))}function me(t){const[r,n]=[...t.values()];return!r||!n?null:Math.hypot(n.x-r.x,n.y-r.y)}function Dt({images:t,initialImageIndex:r,onImageChange:n,onClose:g}){const a=Math.min(Math.max(0,r),Math.max(0,t.length-1)),[l,u]=i.useState(a),[o,f]=i.useState(F),[s,m]=i.useState({x:0,y:0}),[h,j]=i.useState(!1),p=i.useRef(null),I=i.useRef(null),T=i.useRef(null),N=i.useRef(null),E=i.useRef(a),x=i.useRef(F),S=i.useRef({x:0,y:0}),D=i.useRef(null),R=i.useRef(!1),w=i.useRef(new Map),C=i.useRef(null),P=i.useRef(F),A=i.useRef(n),J=i.useRef(g);A.current=n,J.current=g;function ee(c,b=x.current){const v=p.current,M=T.current;if(!v||!M||b<=F)return{x:0,y:0};const H=Math.max(0,(M.clientWidth*b-v.clientWidth)/2),k=Math.max(0,(M.clientHeight*b-v.clientHeight)/2);return{x:Math.min(H,Math.max(-H,c.x)),y:Math.min(k,Math.max(-k,c.y))}}function G(c,b=x.current){const v=ee(c,b);S.current=v,m(v)}function O(c){const b=Math.round(Lt(c)*100)/100;x.current=b,f(b),G(S.current,b)}function z(){x.current=F,S.current={x:0,y:0},f(F),m({x:0,y:0})}function U(c){const b=Math.min(Math.max(0,c),t.length-1);b!==E.current&&(E.current=b,u(b),z(),A.current?.(b))}function B(){J.current(E.current)}i.useEffect(()=>{const c=document.body.style.overflow,b=document.activeElement,v=p.current;document.body.style.overflow="hidden",N.current?.focus();function M(y){if(y.key==="Escape"){y.preventDefault(),B();return}if(y.key==="ArrowLeft"){y.preventDefault(),y.stopPropagation(),U(E.current-1);return}if(y.key==="ArrowRight"){y.preventDefault(),y.stopPropagation(),U(E.current+1);return}if(y.key==="+"||y.key==="="){y.preventDefault(),y.stopPropagation(),O(x.current+le);return}if(y.key==="-"){y.preventDefault(),y.stopPropagation(),O(x.current-le);return}if(y.key==="0"){y.preventDefault(),y.stopPropagation(),z();return}if(y.key==="Tab"){const q=I.current?.querySelectorAll("button:not(:disabled)");if(!q?.length)return;const te=q[0],ne=q[q.length-1],se=document.activeElement;if(y.shiftKey&&se===te){y.preventDefault(),ne.focus();return}if(!y.shiftKey&&se===ne){y.preventDefault(),te.focus();return}I.current?.contains(se)||(y.preventDefault(),te.focus())}}function H(y){if(y.preventDefault(),y.stopPropagation(),y.deltaY===0)return;const q=y.ctrlKey?.01:.002;O(x.current*Math.exp(-y.deltaY*q))}function k(y){y.preventDefault(),y.stopPropagation(),P.current=x.current}function _(y){if(y.preventDefault(),y.stopPropagation(),w.current.size>=2)return;const q=y.scale;typeof q=="number"&&O(P.current*q)}function ae(){G(S.current,x.current)}return document.addEventListener("keydown",M,{capture:!0}),window.addEventListener("resize",ae),v?.addEventListener("wheel",H,{passive:!1}),v?.addEventListener("gesturestart",k,{passive:!1}),v?.addEventListener("gesturechange",_,{passive:!1}),v?.addEventListener("gestureend",_,{passive:!1}),()=>{document.removeEventListener("keydown",M,{capture:!0}),window.removeEventListener("resize",ae),v?.removeEventListener("wheel",H),v?.removeEventListener("gesturestart",k),v?.removeEventListener("gesturechange",_),v?.removeEventListener("gestureend",_),document.body.style.overflow=c,b instanceof HTMLElement&&b.focus()}},[]),i.useEffect(()=>{[t[l-1],t[l+1]].forEach(c=>{if(!c)return;const b=new Image;b.src=c.src})},[l,t]);function Q(c,b,v){D.current={pointerId:c,startX:b,startY:v,originX:S.current.x,originY:S.current.y},j(!0)}function Y(c){if(c.target instanceof Element&&c.target.closest("button, .thread-image-lightbox-controls"))return;const b=c.pointerType==="touch",v=c.pointerType==="mouse"&&c.button===0;if(!(!b&&!v)&&(R.current=!1,!(!b&&x.current<=F))){if(c.preventDefault(),c.currentTarget.setPointerCapture(c.pointerId),b&&(w.current.set(c.pointerId,{x:c.clientX,y:c.clientY}),w.current.size===2)){C.current=me(w.current),D.current=null,j(!1);return}x.current>F&&Q(c.pointerId,c.clientX,c.clientY)}}function X(c){const b=w.current.has(c.pointerId),v=D.current;if(!b&&v?.pointerId!==c.pointerId)return;if(c.preventDefault(),c.stopPropagation(),b&&w.current.set(c.pointerId,{x:c.clientX,y:c.clientY}),w.current.size===2){const k=me(w.current),_=C.current;if(!k||!_){C.current=k;return}Math.abs(k-_)>1&&(R.current=!0),O(x.current*(k/_)),C.current=k;return}if(!v||x.current<=F)return;const M=c.clientX-v.startX,H=c.clientY-v.startY;Math.hypot(M,H)>3&&(R.current=!0),G({x:v.originX+M,y:v.originY+H})}function $(c){const b=w.current.delete(c.pointerId),v=D.current?.pointerId===c.pointerId;if(!(!b&&!v)){if(C.current=w.current.size===2?me(w.current):null,w.current.size===1&&x.current>F){const[M]=w.current.entries();if(M){const[H,k]=M;Q(H,k.x,k.y)}}else D.current=null,j(!1);c.currentTarget.hasPointerCapture(c.pointerId)&&c.currentTarget.releasePointerCapture(c.pointerId)}}const V=Math.round(o*100),K=t[l]??t[0];return K?Ke.createPortal(e.jsx("div",{className:"thread-image-lightbox-backdrop","data-can-pan":o>F,"data-dragging":h,onClick:c=>{c.target===c.currentTarget&&!R.current&&B()},onPointerCancel:$,onPointerDown:Y,onPointerMove:X,onPointerUp:$,ref:p,role:"presentation",children:e.jsxs("figure",{"aria-label":K.alt?`图片预览：${K.alt}（${l+1}/${t.length}）`:`图片预览（${l+1}/${t.length}）`,"aria-modal":"true",className:"thread-image-lightbox",ref:I,role:"dialog",children:[e.jsx("button",{"aria-label":"关闭图片预览",className:"thread-image-lightbox-close",onClick:B,ref:N,type:"button",children:e.jsx(re,{size:20})}),t.length>1&&e.jsxs(e.Fragment,{children:[e.jsx("button",{"aria-label":"上一张图片",className:"thread-image-lightbox-nav thread-image-lightbox-prev",disabled:l===0,onClick:()=>U(l-1),title:"上一张（←）",type:"button",children:e.jsx(We,{size:28})}),e.jsx("button",{"aria-label":"下一张图片",className:"thread-image-lightbox-nav thread-image-lightbox-next",disabled:l===t.length-1,onClick:()=>U(l+1),title:"下一张（→）",type:"button",children:e.jsx(Xe,{size:28})})]}),e.jsx("img",{alt:K.alt,draggable:"false",onLoad:()=>G(S.current,x.current),ref:T,src:K.src,style:{transform:`translate3d(${s.x}px, ${s.y}px, 0) scale(${o})`}}),K.alt&&e.jsx("figcaption",{children:K.alt}),e.jsxs("div",{"aria-label":"图片缩放",className:"thread-image-lightbox-controls",role:"toolbar",children:[e.jsx("button",{"aria-label":"缩小图片",disabled:o<=F,onClick:()=>O(o-le),title:"缩小（-）",type:"button",children:e.jsx(dt,{size:18})}),e.jsxs("output",{"aria-label":"当前缩放比例","aria-live":"polite",children:[V,"%"]}),e.jsx("button",{"aria-label":"放大图片",disabled:o>=Te,onClick:()=>O(o+le),title:"放大（+）",type:"button",children:e.jsx(xt,{size:18})}),e.jsx("button",{"aria-label":"恢复原始大小",disabled:o===F,onClick:z,title:"恢复原始大小（0）",type:"button",children:e.jsx(bt,{size:17})})]})]})}),document.body):null}const Mt="/assets/thread-html-frame-DDSDNjxO.css",Pt=28,Ft=64,Ht=5e4,Ot=30,Ce=30,Z="capubbs-thread-html-frame",zt=new URL(Mt,window.location.origin).href;function we({className:t="",floor:r,html:n,isActivitySignupCanceled:g=!1,onImageOpen:a,variant:l}){const u=i.useMemo(()=>l==="signature"?ut(n):n,[n,l]),o=qt(u,l==="signature"),f=jt(o),s=i.useMemo(()=>f?null:It(o,{normalizeLegacyLineBreaks:l==="signature"}),[o,f,l]),m=i.useMemo(()=>wt(o),[o]);return!f&&s!==null?e.jsx(ke,{className:t,html:s,onImageOpen:a,variant:l}):e.jsx(_t,{className:t,floor:r,html:m,isActivitySignupCanceled:g,onImageOpen:a,variant:l})}function _t({className:t,floor:r,html:n,isActivitySignupCanceled:g,onImageOpen:a,variant:l}){const u=i.useRef(null),o=i.useRef(`${l}-${r}-${Math.random().toString(36).slice(2)}`),f=i.useRef(a);f.current=a;const s=l==="signature"?Pt:Ft,m=!!a,[h,j]=i.useState(null),p=Wt(),I=i.useRef(p),T=Ve(),N=l==="signature"?14:T,E=i.useMemo(()=>Gt({canOpenImages:m,frameId:o.current,html:n,isActivitySignupCanceled:g,isDarkTheme:I.current,fontSize:N,variant:l}),[m,N,n,g,l]),x=i.useMemo(()=>`data:text/html;charset=utf-8,${encodeURIComponent(E)}`,[E]),S=i.useCallback(()=>{u.current?.contentWindow?.postMessage({frameId:o.current,source:Z,theme:p?"dark":"light",type:"theme"},"*")},[p]);return i.useEffect(()=>{j(null)},[x]),i.useEffect(()=>{S()},[S]),i.useLayoutEffect(()=>{function D(R){if(!(R.source!==u.current?.contentWindow||!Kt(R.data))&&R.data.frameId===o.current){if(R.data.type==="anchor"){const w=u.current;if(!w)return;const C=window.getComputedStyle(document.documentElement),P=Number.parseFloat(C.getPropertyValue("--topbar-height"))||0,A=window.scrollY+w.getBoundingClientRect().top;window.scrollTo({left:0,top:Math.max(0,A+R.data.offsetTop-P-16)});return}if(R.data.type==="navigate"){const w=Je(R.data.url,$e());if(!w)return;window.history.pushState(null,"",w),window.dispatchEvent(new Event(Qe));const C=new URL(w,window.location.origin);C.hash?window.requestAnimationFrame(()=>{const P=decodeURIComponent(C.hash.slice(1)),A=Ze(`#${P}`);(A?et(A):document.getElementById(P))?.scrollIntoView({block:"start"})}):window.scrollTo({left:0,top:0});return}if(R.data.type==="image-open"){const w=u.current;if(!w)return;const C=P=>{const A=R.data.images[P];!A||typeof A.galleryId!="number"||!Number.isSafeInteger(A.galleryIndex)||w.contentWindow?.postMessage({frameId:o.current,galleryId:A.galleryId,galleryIndex:A.galleryIndex,source:Z,type:"gallery-select"},"*")};f.current?.(R.data.images,R.data.imageIndex,w,C);return}j(Math.min(Ht,Math.max(s,Math.ceil(R.data.height))))}}return window.addEventListener("message",D),()=>window.removeEventListener("message",D)},[s]),e.jsx("iframe",{ref:u,className:`thread-html-frame thread-html-frame-${l} ${t}`.trim(),referrerPolicy:"no-referrer",sandbox:"allow-scripts allow-same-origin",scrolling:"no",src:x,onLoad:S,style:{"--thread-html-frame-width-allowance":`${Ce}px`,...h===null?{}:{"--thread-html-frame-height":`${h}px`}},title:l==="signature"?`第 ${r} 楼签名档`:`第 ${r} 楼正文`})}function qt(t,r){const[n,g]=i.useState(t);return i.useEffect(()=>{const a=new AbortController,l=r?mt(t):[];if(g(t),l.length===0)return()=>a.abort();const u=Array.from(new Map(l.map(o=>[`${o.bid}:${o.tid}:${o.pid}`,o])).values());return Promise.all(u.map(async o=>{try{const f=await vt(o,a.signal);return[`${o.bid}:${o.tid}:${o.pid}`,f]}catch(f){if(f instanceof DOMException&&f.name==="AbortError")throw f;return[`${o.bid}:${o.tid}:${o.pid}`,""]}})).then(o=>{if(a.signal.aborted)return;const f=new Map(o);let s=t;l.forEach(m=>{const h=f.get(`${m.bid}:${m.tid}:${m.pid}`);h&&(s=s.replace(m.marker,h))}),g(s)}).catch(()=>{}),()=>a.abort()},[r,t]),n}function Gt({canOpenImages:t,frameId:r,fontSize:n,html:g,isActivitySignupCanceled:a,isDarkTheme:l,variant:u}){const o=u==="signature",f=o?"#999999":"rgb(63 63 70)",s=o?"#999999":"rgb(228 228 231)",m=o?"monospace":"ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",h=o?"padding-top:10px;color:inherit;font-family:inherit;font-size:inherit;":"",j=a?" capubbs-activity-signup-canceled":"";return`<!doctype html>
