...
const preloadSource = readFileSync(resolve(forumDirectory, '../bbs/index-preload.js'), 'utf8');
...
assert.match(workerHtaccess, /Cache-Control "no-cache"/);
assert.match(gatewaySource, /\$_GET\['capubbs_shell'\] === 'new'/);
assert.match(gatewaySource, /\$hasLegacyLoginToken = !\$forceNewShell/);
...
assert.match(viteConfigSource, /manifest: 'new-assets\/precache-manifest\.json'/);
...
assert.match(oldForumSource, /openNewForum\(event\)/);
assert.match(oldForumSource, /capubbsPrepareForumShell\('new'\)/);
assert.match(oldForumSource, /src="\/bbs\/index-preload\.js"/);
assert.match(oldForumSource, /<base href="\/bbs\/index\/">/);
assert.match(siteHomeSource, /href="\/bbs\/"/);
assert.match(siteHomeSource, /src="\/bbs\/index-preload\.js"/);
assert.match(preloadSource, /updateViaCache: 'none'/);
assert.match(preloadSource, /PREPARE_FORUM_SHELL/);
assert.doesNotMatch(preloadSource, /navigator\.serviceWorker\.ready/);
