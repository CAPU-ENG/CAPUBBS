...
资源请求失败或连续 60 秒没有进展时提供重新加载按钮。开发模式使用独立的滚动加载条，不显示百分比，因为源码模块没有构建后的固定字节清单；失败后停止滚动，并遵循系统减少动态效果的偏好。需要验证真实字节进度时，先构建，再通过本地 PHP 服务访问新论坛页面。增加或修改 `App.tsx` 中的懒加载页面时，同步维护 `build/startupLoading.ts` 的 `pageRoutes`；使用内层 Suspense 的页面需在该边界内保留 `StartupReady`，以免加载界面过早退出。
