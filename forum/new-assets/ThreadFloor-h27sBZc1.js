...
<head>
  ...
  <style>
    ...
    .capubbs-html-frame-root{display:flow-root;width:calc(100% - ${Qe}px);${b}}.capubbs-html-frame-root iframe{display:inline-block;vertical-align:baseline}
  </style>
  <script>${jr(t,e,a,r)}<\/script>
</head>
<body><main class="capubbs-html-frame-root forum-markup forum-markup-${u}${x}">${l}</main></body>
</html>`}function jr(e,r,t,o=!1){return`(function(){
    var frameId=${JSON.stringify(e)};
    var forumOrigin=${JSON.stringify(window.location.origin)};
    var forumBasePath=${JSON.stringify(St)};
    var canOpenImages=${JSON.stringify(r)};
    var canQuoteImages=${JSON.stringify(o)};
    var ensureGalleryQuoteControls=${We.toString()};
    var needsJquery=${JSON.stringify(t)};
    var preparePunishmentTableFit=${Je.toString()};
    var getGalleryImageState=${kt.toString()};
    ...
    var forumAppExactPaths=${JSON.stringify(Rt)};
    var forumAppPathPrefixes=${JSON.stringify(Et)};
    var legacyForumExactPaths=${JSON.stringify(Ct)};
    var legacyForumPathPatterns=${JSON.stringify(jt)}.map(function(pattern){return new RegExp(pattern);});
    var minBottomGuard=${Ar};
    var queued=false;
  ...
  }());`}function qr(e){return e.replace(/<script\b([^>]*)>/gi,(r,t)=>`<script${t.replace(/\s+type\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/gi,"")} type="text/capubbs-user-script">`)}function Nr(e){if(!/<(?:img|iframe)\b/i.test(e))return e;const r=document.createElement("template");return r.innerHTML=e,r.content.querySelectorAll("iframe[src]").forEach(t=>{const o=xe(t.getAttribute("src")??"",ve());o&&(t.dataset.capubbsPlayerSrc=o,t.removeAttribute("src"))}),r.content.querySelectorAll("img[src]").forEach(t=>{const o=t.getAttribute("src")?.trim()??"";!o||/^(?:blob:|data:)/i.test(o)||(t.dataset.capubbsImageResourceSrc=o,t.setAttribute("fetchpriority","low"),t.removeAttribute("src"),t.removeAttribute("srcset"),t.closest("picture")?.querySelectorAll("source[srcset]").forEach(a=>{a.removeAttribute("srcset")}))}),r.innerHTML}function Oe(){return oe||(oe=fetch(Xe,{credentials:"same-origin"}).then(e=>{if(!e.ok)throw new Error(`Failed to load jQuery: ${e.status}`);return e.text()}).catch(()=>null),oe)}function Tr(){return["default-src 'none'","script-src 'unsafe-inline' http: https: data: blob:","style-src 'unsafe-inline' http: https:","img-src http: https: data: blob:","media-src http: https: data: blob:","font-src http: https: data: blob:","frame-src http: https: data: blob:","child-src http: https: data: blob:","connect-src 'none'","object-src 'none'","form-action 'none'","upgrade-insecure-requests"].join("; ")}function ve(){return new URL("/bbs/content/",window.location.origin).href}function Lr(e){return e.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function $r(e){return e.replace(/<\/style/gi,"<\\/style")}function Pr(e){if(!e||typeof e!="object")return!1;const r=e;if(r.source!==E||typeof r.frameId!="string")return!1;if(r.type==="image-quote"){const t=r.image;if(!t||typeof t.src!="string"||typeof t.title!="string"||typeof t.caption!="string")return!1;try{return["http:","https:"].includes(new URL(t.src).protocol)}catch{return!1}}return r.type==="embedded-player-layout"?Array.isArray(r.players)&&r.players.every(gr):r.type==="document-request"?typeof r.token=="string":r.type==="anchor"?typeof r.offsetTop=="number"&&Number.isFinite(r.offsetTop)&&r.offsetTop>=0:r.type==="navigate"?typeof r.url=="string":r.type==="jquery-request"?!0:r.type==="image-resource-request"||r.type==="image-resource-layout"?typeof r.requestId=="string"&&r.requestId.length>0&&Array.isArray(r.bounds)&&r.bounds.every(t=>t&&(t.gallery===void 0||["current","adjacent","deferred"].includes(t.gallery))&&["top","bottom","left","right"].every(o=>typeof t[o]=="number"&&Number.isFinite(t[o])))&&(r.type==="image-resource-layout"||"url"in r&&typeof r.url=="string"&&r.url.length>0):r.type==="selection"?typeof r.text=="string":r.type==="image-open"?typeof r.imageIndex=="number"&&Number.isSafeInteger(r.imageIndex)&&Array.isArray(r.images)&&r.images.length>0&&r.imageIndex>=0&&r.imageIndex<r.images.length&&r.images.every(t=>!!t&&typeof t=="object"&&typeof t.alt=="string"&&typeof t.elementIndex=="number"&&Number.isSafeInteger(t.elementIndex)&&t.elementIndex>=0&&typeof t.src=="string"&&t.src.length>0&&(t.galleryId===void 0&&t.galleryIndex===void 0||typeof t.galleryId=="number"&&Number.isSafeInteger(t.galleryId)&&t.galleryId>=0&&typeof t.galleryIndex=="number"&&Number.isSafeInteger(t.galleryIndex)&&t.galleryIndex>=0)):r.type==="resize"&&typeof r.height=="number"&&Number.isFinite(r.height)}function Mr(){const[e,r]=d.useState(()=>document.documentElement.classList.contains("dark"));return d.useEffect(()=>{const t=document.documentElement,o=()=>r(t.classList.contains("dark")),a=new MutationObserver(o);return a.observe(t,{attributeFilter:["class"],attributes:!0}),()=>a.disconnect()},[]),e}function Fr({attachments:e=[],bodyClassName:r="thread-floor-body",bodyFallback:t=null,bodyHtml:o,floor:a,isActivitySignupCanceled:l=!1,onImageOpen:i,onImageQuote:s,onIsolatedTextSelection:u,signatureClassName:p="thread-signature",signatureHtml:m,signatureText:h}){const c=i?(b,x,f,y)=>{const w=b[x];w&&i([w],0,f,y?()=>y(x):void 0)}:void 0;return n.jsxs(n.Fragment,{children:[o?n.jsx(ze,{className:r,floor:a,html:o,isActivitySignupCanceled:l,onImageOpen:i,onImageQuote:s,onIsolatedTextSelection:u,variant:"floor"}):t,n.jsx(zr,{attachments:e}),m?n.jsx(ze,{className:p,floor:a,html:m,onImageOpen:c,variant:"signature"}):h?n.jsx("footer",{className:p,children:n.jsx("p",{children:h})}):null]})}function zr({attachments:e}){return e.length===0?null:n.jsxs("section",{"aria-label":"附件",className:"thread-attachments",children:[n.jsxs("header",{className:"thread-attachments-heading",children:[n.jsx(Wt,{"aria-hidden":"true",size:14}),n.jsx("span",{children:"附件"}),n.jsx("small",{children:e.length})]}),n.jsx("ul",{children:e.map(r=>{const t=n.jsxs(n.Fragment,{children:[n.jsx("span",{className:"thread-attachment-name",children:r.name}),n.jsx("small",{children:Or(r)}),r.exists!==!1&&n.jsx(qt,{"aria-hidden":"true",size:15})]});return n.jsx("li",{children:r.exists===!1?n.jsx("div",{"aria-disabled":"true",className:"thread-attachment-link is-unavailable",children:t}):n.jsx("a",{className:"thread-attachment-link",download:r.name,href:r.downloadHref||`/bbs/download/?id=${encodeURIComponent(r.id)}`,children:t})},r.id)})})]})}function Or(e){if(e.exists===!1)return"文件不可用";const r=[Dr(e.size),(e.price??0)>0?"付费附件":"免费"];return e.downloadCount!==void 0&&r.push(`下载 ${e.downloadCount} 次`),r.join(" · ")}function Dr(e){if(e<=0)return"大小未知";if(e<1024)return`${e} B`;const r=["KB","MB","GB","TB"];let t=e,o=-1;do t/=1024,o+=1;while(t>=1024&&o<r.length-1);return`${t.toFixed(t>=10?1:2)} ${r[o]}`}function Hr({author:e,id:r}){const t=e.tags??[],[o,a]=d.useState(!1),l=d.useRef(null),i=d.useRef(null),s=d.useRef(null),u=d.useRef(null),p=t.map(m=>`${m.id}:${m.name}`).join("|");return d.useLayoutEffect(()=>{if(t.length===0){a(!1);return}const m=()=>{const c=l.current,b=i.current,x=s.current,f=u.current;if(!c||!b||!x||!f||c.offsetWidth===0)return;const y=x.getBoundingClientRect().width,w=f.getBoundingClientRect().width,C=Number.parseFloat(getComputedStyle(b).columnGap)||0,H=b.clientWidth-y-C,T=w>H+1;a(I=>I===T?I:T)};m();const h=new ResizeObserver(m);return[l.current,i.current,u.current].forEach(c=>{c&&h.observe(c)}),()=>h.disconnect()},[p,t.length]),n.jsxs("div",{id:r,ref:l,className:"author-hover-card",role:"dialog","aria-label":`${e.name} 的用户摘要`,children:[n.jsxs("div",{className:"author-card-head",children:[n.jsx("img",{src:e.avatar,alt:""}),n.jsxs("div",{className:"author-card-head-copy",children:[n.jsxs("div",{ref:i,className:"author-card-name-line","data-tags-overflow":o?"true":void 0,children:[n.jsx("strong",{ref:s,children:e.name}),n.jsx("div",{className:"author-card-tag-slot",children:n.jsx(ce,{size:"compact",tags:t})})]}),(e.stars>0||e.role)&&n.jsxs("span",{className:"author-card-status",children:["★".repeat(e.stars),e.stars>0&&e.role?" · ":"",e.role]})]})]}),o?n.jsx("div",{className:"author-card-tags-row",children:n.jsx(ce,{size:"compact",tags:t})}):null,e.medals?.length?n.jsx("div",{className:"author-card-medals",children:n.jsx(Be,{medals:e.medals,profileName:e.name,variant:"compact"})}):null,n.jsx("div",{ref:u,className:"author-card-tag-width-measure","aria-hidden":"true",children:n.jsx(ce,{size:"compact",tags:t})}),n.jsxs("dl",{children:[n.jsxs("div",{children:[n.jsx("dt",{children:"主题"}),n.jsx("dd",{children:e.topics})]}),n.jsxs("div",{children:[n.jsx("dt",{children:"回复"}),n.jsx("dd",{children:e.replies})]}),n.jsxs("div",{children:[n.jsx("dt",{children:"签到"}),n.jsx("dd",{children:e.checkins})]})]}),n.jsxs("p",{children:["最近在线：",e.lastSeen]}),n.jsxs("a",{href:te(e.name),children:["查看个人主页 ",n.jsx(Pt,{size:13})]})]})}function Ur({author:e}){const r=e.tags??[],t=_e(r),o=te(e.name);return n.jsxs("aside",{className:"thread-author-profile","aria-label":`${e.name} 的资料`,children:[n.jsx("a",{"aria-label":`查看${e.name}的个人主页`,className:"thread-author-profile-avatar",href:o,children:n.jsx("img",{src:e.avatar,alt:""})}),n.jsx("div",{className:"thread-author-profile-identity",children:n.jsx("a",{href:o,children:e.name})}),(e.stars>0||e.role)&&n.jsxs("div",{className:"thread-author-profile-status",children:[e.stars>0&&n.jsx("span",{"aria-label":`${e.stars} 星`,children:"★".repeat(e.stars)}),e.role&&n.jsx("strong",{children:e.role})]}),n.jsx(Ge,{tags:t}),n.jsx(Be,{medals:e.medals??[],profileName:e.name,variant:"compact"}),n.jsxs("dl",{className:"thread-author-profile-stats",children:[n.jsxs("div",{children:[n.jsx("dt",{children:"主题"}),n.jsx("dd",{children:e.topics})]}),n.jsxs("div",{children:[n.jsx("dt",{children:"回复"}),n.jsx("dd",{children:e.replies})]}),n.jsxs("div",{children:[n.jsx("dt",{children:"签到"}),n.jsx("dd",{children:e.checkins})]})]}),n.jsxs("p",{className:"thread-author-profile-last-seen",children:[n.jsx("span",{children:"最近在线"}),n.jsx("strong",{children:e.lastSeen})]})]})}function be(e){return e.replace(/^(\d{4})年(\d{2})月(\d{2})日\s+(\d{2})时(\d{2})分(\d{2})秒$/,"$1-$2-$3 $4:$5:$6")}function Gr(e){const r=window.getSelection()?.toString();r&&(e.preventDefault(),e.clipboardData.setData("text/plain",r))}function _r({articleAfterContent:e,author:r,avatarRail:t,className:o="",content:a,decorationImageSrc:l,editedAt:i,floor:s,floorIndex:u,id:p,inlineAvatar:m=!1,mainAfterContent:h,onCopy:c,publishedAt:b,showAuthorProfile:x}){const f=r.tags??[],y=_e(f);return n.jsxs("article",{className:`thread-floor${x?" thread-floor-with-author-profile":""}${o?` ${o}`:""}`,"data-floor":s,id:p,onCopy:c,children:[l&&n.jsx("span",{"aria-hidden":"true",className:"thread-floor-decoration",children:n.jsx("img",{alt:"",src:l})}),x?n.jsx(Ur,{author:r}):!m&&t,n.jsxs("div",{className:"thread-floor-main",children:[n.jsxs("header",{className:"thread-floor-header",children:[!x&&m&&t,n.jsxs("div",{className:"thread-floor-author",children:[n.jsx("a",{href:te(r.name),children:r.name}),n.jsx(Ge,{tags:y})]}),n.jsxs("div",{className:"thread-floor-time",children:[n.jsx("time",{children:be(b)}),i&&n.jsxs(n.Fragment,{children:[n.jsx("span",{children:"·"}),n.jsxs("time",{children:["编辑于 ",be(i)]})]})]}),u]}),x?n.jsx("div",{className:"thread-floor-content",children:a}):a,h]}),e]})}function Br({canDelete:e,canEdit:r,canQuote:t,canReply:o,decorative:a=!1,deleting:l=!1,editHref:i="",onDelete:s,onEditSignup:u,onQuote:p,onReply:m}){const h=a?-1:void 0,c=d.useRef(null);return n.jsxs("div",{"aria-hidden":a||void 0,className:`thread-floor-actions${a?" thread-floor-actions-decorative":""}`,children:[t&&n.jsxs("button",{onClick:b=>{const x=c.current?c.current.text:De(b.currentTarget);c.current=null,p?.(x)},onPointerDown:b=>{b.button===0&&(c.current={text:De(b.currentTarget)})},tabIndex:h,type:"button",children:[n.jsx($t,{size:15}),"引用"]}),u&&!a&&n.jsxs("button",{onClick:u,type:"button",children:[n.jsx(de,{size:15}),"编辑报名"]}),o&&n.jsxs("button",{onClick:m,tabIndex:h,type:"button",children:[n.jsx(Jt,{size:15}),"回复"]}),r&&(a?n.jsxs("button",{tabIndex:-1,type:"button",children:[n.jsx(de,{size:15}),"编辑"]}):n.jsxs("a",{href:i,children:[n.jsx(de,{size:15}),"编辑"]})),e&&n.jsxs("button",{"aria-busy":l||void 0,className:"floor-action-danger",disabled:!a&&l,onClick:a?void 0:b=>s?.(b.currentTarget),tabIndex:h,type:"button",children:[n.jsx(ye,{size:15}),l?"删除中":"删除"]})]})}function ra({canQuote:e,canReply:r,decorationImageSrc:t,editHref:o,floor:a,isActivityThread:l,isMainPost:i,locked:s,inlineAvatar:u,showAuthorProfile:p,hideSignature:m,onDeleteFloor:h,onDeleteNestedReply:c,onEditSignup:b,onIsolatedTextSelection:x,onQuote:f,onSubmitNestedReply:y,viewer:w}){const[C,H]=d.useState(!1),[T,I]=d.useState(null),[F,U]=d.useState([]),[W,$]=d.useState(""),[Q,N]=d.useState(!1),[X,V]=d.useState([]),[G,_]=d.useState(""),[ae,B]=d.useState(""),[k,P]=d.useState(null),[Z,v]=d.useState(""),[D,S]=d.useState(!1),[R,q]=d.useState(void 0),A=Qt(G),M=Nt(":scope > article"),L=`nested-reply-count-${a.id}`,[J,we]=d.useState(null),[ne,Ie]=d.useState(!1),Ae=d.useRef(null),ee=d.useRef(null),ie=d.useRef(null),Se=d.useRef(null),ke=d.useRef(null),Re=d.useMemo(()=>[...a.nestedReplies??[],...X].filter(g=>!F.includes(g.id)),[F,a.nestedReplies,X]),Ee=l&&!i&&/<\s*(?:s|strike)\b/i.test(a.contentHtml??""),Ce=`thread-floor-body${Ee?" capubbs-activity-signup-canceled":""}`;d.useEffect(()=>()=>{ee.current!==null&&window.clearTimeout(ee.current)},[]),d.useEffect(()=>{if(!ne)return;function g(j){Ae.current?.contains(j.target)||Ie(!1)}return document.addEventListener("pointerdown",g),()=>document.removeEventListener("pointerdown",g)},[ne]);async function Ze(){const g=`${window.location.origin}${window.location.pathname}${window.location.search}#${a.floor}`;await Kt(g)&&(H(!0),ee.current!==null&&window.clearTimeout(ee.current),ee.current=window.setTimeout(()=>H(!1),1800))}const je=(g,j,z,le)=>{ke.current=z,we({imageIndex:j,images:g,onImageChange:le})};function et(g){J?.onImageChange?.(g),we(null),window.requestAnimationFrame(()=>ke.current?.focus())}function qe(g=null){q(g),_(""),B(""),v(""),window.requestAnimationFrame(()=>Se.current?.focus())}function Ne(){q(void 0),_(""),v("")}async function tt(g){g.preventDefault();const j=G.trim();if(!(!A.canSubmit||!w||!r||D)){S(!0),v("");try{const z=await y(a,R??null,j);V(le=>[...le,{author:w,canDelete:!0,content:j,id:z>0?String(z):`local-${a.id}-${Date.now()}`,publishedAt:Jr(new Date),target:R??void 0}]),Ne()}catch(z){v(z instanceof Error?z.message:"楼中楼回复发布失败，请稍后重试。")}finally{S(!1)}}}async function rt(g){P(g.id),B("");try{await c(a,g),U(j=>[...j,g.id]),V(j=>j.filter(z=>z.id!==g.id)),I(null)}catch(j){B(j instanceof Error?j.message:"楼中楼删除失败，请稍后重试。")}finally{P(null)}}async function at(){if(!Q){N(!0),$("");try{await h(a)}catch(g){$(g instanceof Error?g.message:"楼层删除失败，请稍后重试。"),N(!1)}}}function nt(){I(null),$(""),B(""),window.requestAnimationFrame(()=>ie.current?.focus())}function ot(){if(s||!T)return;const g=T;I(null),g.kind==="floor"?at():rt(g.reply)}const st=n.jsxs("div",{className:`thread-avatar-rail${ne?" thread-avatar-rail-open":""}`,ref:Ae,children:[n.jsx("button",{"aria-controls":`author-card-${a.floor}`,"aria-expanded":ne,"aria-label":`查看${a.author.name}的资料卡`,className:"thread-avatar-button",onClick:()=>Ie(g=>!g),type:"button",children:n.jsx("img",{src:a.author.avatar,alt:""})}),n.jsx(Hr,{author:a.author,id:`author-card-${a.floor}`})]}),it=n.jsx(Fr,{attachments:a.attachments,bodyFallback:n.jsx("div",{className:Ce,children:a.paragraphs.map(g=>n.jsx("p",{children:g},g))}),bodyClassName:Ce,bodyHtml:a.contentHtml,floor:a.floor,isActivitySignupCanceled:Ee,onImageOpen:je,onImageQuote:e?g=>f(a,void 0,g):void 0,onIsolatedTextSelection:g=>x(a,g),signatureHtml:m?void 0:a.signatureHtml,signatureText:m?void 0:a.signature}),lt=n.jsxs("button",{"aria-label":`复制第 ${a.floor} 楼链接`,className:"thread-floor-index",onClick:Ze,title:"复制楼层链接",type:"button",children:["#",a.floor]}),ct=n.jsxs(n.Fragment,{children:[n.jsx(Br,{canDelete:!s&&(!l||i)&&(a.canDelete??a.isOwn??!1),canEdit:!s&&(!l||i)&&!!a.isOwn,canQuote:e,canReply:r,deleting:Q,editHref:o,onEditSignup:!s&&l&&!i&&a.isOwn?b:void 0,onDelete:g=>{ie.current=g,$(""),I({kind:"floor"})},onQuote:g=>f(a,g),onReply:()=>qe()}),W&&n.jsx("p",{className:"thread-floor-delete-error",role:"alert",children:W}),Re.length>0&&n.jsx("section",{className:"nested-replies",ref:M,"aria-label":`${a.floor} 楼的楼中楼回复`,children:Re.map(g=>n.jsxs("article",{children:[n.jsx("img",{src:g.author.avatar,alt:""}),n.jsxs("div",{className:"nested-reply-main",children:[n.jsxs("div",{className:"nested-reply-identity",children:[n.jsx("a",{className:"nested-reply-author",href:te(g.author.name),children:g.author.name}),g.target&&n.jsxs("span",{className:"nested-reply-target",children:[" ","回复"," ",n.jsx("a",{className:"nested-reply-author",href:te(g.target),children:g.target})]})]}),g.contentHtml?n.jsx(Ke,{className:"nested-reply-content",html:g.contentHtml,onImageOpen:je,variant:"nested"}):n.jsx("p",{children:g.content}),n.jsxs("footer",{className:"nested-reply-footer",children:[n.jsx("time",{children:be(g.publishedAt)}),r&&n.jsx("button",{onClick:()=>qe(g.author.name),type:"button",children:"回复"}),!s&&g.canDelete&&n.jsxs("button",{className:"nested-reply-delete",disabled:k===g.id,onClick:j=>{ie.current=j.currentTarget,B(""),I({kind:"nested",reply:g})},type:"button",children:[n.jsx(ye,{size:12}),k===g.id?"删除中":"删除"]})]})]})]},g.id))}),ae&&n.jsx("p",{className:"nested-reply-delete-error",role:"alert",children:ae}),R!==void 0&&r&&n.jsxs("form",{className:"nested-reply-composer",onSubmit:tt,children:[n.jsxs("div",{className:"nested-reply-input-field",children:[n.jsx("textarea",{"aria-describedby":L,"aria-invalid":A.isOverLimit||void 0,"aria-label":R?`回复 @${R}`:`回复第 ${a.floor} 楼`,onChange:g=>{_(g.target.value),v("")},placeholder:R?`回复 @${R}`:"写一条楼中楼回复",ref:Se,rows:2,value:G}),n.jsxs("small",{"aria-label":`已输入 ${A.length} 字，最多 ${A.limit} 字`,className:`nested-reply-character-count${A.isOverLimit?" nested-reply-character-count-error":""}`,id:L,children:[A.length," / ",A.limit]})]}),n.jsxs("div",{className:"nested-reply-composer-actions",children:[n.jsx("button",{"aria-label":"取消楼中楼回复",className:"nested-reply-cancel",disabled:D,onClick:Ne,type:"button",children:n.jsx(Ue,{size:15})}),n.jsxs("button",{className:"nested-reply-submit",disabled:!A.canSubmit||D,type:"submit",children:[n.jsx(Tt,{size:14}),D?"发送中":"发送"]})]}),Z&&n.jsx("p",{className:"nested-reply-error",role:"alert",children:Z})]})]}),ut=n.jsxs(n.Fragment,{children:[C&&n.jsxs("div",{"aria-live":"polite",className:"copy-floor-toast",role:"status",children:[n.jsx(Lt,{"aria-hidden":"true",size:15}),"已复制楼层链接"]}),n.jsx(Te,{children:J&&n.jsx(Ut,{images:J.images,initialImageIndex:J.imageIndex,onImageChange:J.onImageChange,onClose:et})}),n.jsx(Te,{mobileSize:"compact",children:!s&&T&&n.jsx(Wr,{floor:a,isMainPost:i,onCancel:nt,onConfirm:ot,target:T})})]});return n.jsx(_r,{articleAfterContent:ut,author:a.author,avatarRail:st,content:it,decorationImageSrc:t,editedAt:a.editedAt,floor:a.floor,floorIndex:lt,id:String(a.floor),inlineAvatar:u,mainAfterContent:ct,onCopy:Gr,publishedAt:a.publishedAt,showAuthorProfile:p})}function De(e){const r=e.closest(".thread-floor")?.querySelector(".thread-floor-body");return Ft(window.getSelection(),r??null)}function Wr({floor:e,isMainPost:r,onCancel:t,onConfirm:o,target:a}){const l=a.kind==="nested"?a.reply:null,i=l?"删除楼中楼回复":r?"删除主楼":"删除回复",s=l?"":r?"删除主楼后，下一楼将顺位成为主楼；如果没有其他回复，整个主题会被删除。":"删除后，该楼内容将移入回收站，后续楼层编号会顺次调整。",u=l?.author.name??e.author.name,p=l?`#${e.floor} · 楼中楼`:`#${e.floor}`,m=Vr(l?.content||e.quoteText||e.paragraphs[0]||"");return d.useEffect(()=>(document.body.classList.add("thread-delete-dialog-open"),()=>document.body.classList.remove("thread-delete-dialog-open")),[]),d.useEffect(()=>{function h(c){c.key==="Escape"&&t()}return document.addEventListener("keydown",h),()=>document.removeEventListener("keydown",h)},[t]),n.jsx(Mt,{className:"thread-delete-dialog-backdrop",onMouseDown:h=>{h.currentTarget===h.target&&t()},role:"presentation",children:n.jsxs("section",{"aria-describedby":s?"thread-delete-dialog-description":void 0,"aria-labelledby":"thread-delete-dialog-title","aria-modal":"true",className:"thread-delete-dialog",role:"dialog",children:[n.jsxs("header",{children:[n.jsx("span",{className:"thread-delete-dialog-icon","aria-hidden":"true",children:n.jsx(_t,{size:19})}),n.jsx("div",{children:n.jsx("h2",{id:"thread-delete-dialog-title",children:i})}),n.jsx("button",{"aria-label":"关闭删除确认",onClick:t,type:"button",children:n.jsx(Ue,{size:18})})]}),n.jsxs("div",{className:"thread-delete-dialog-body",children:[s&&n.jsx("p",{id:"thread-delete-dialog-description",children:s}),n.jsxs("div",{className:"thread-delete-dialog-target",children:[n.jsxs("span",{children:[u," · ",p]}),n.jsx("p",{children:m||"此回复没有可预览的文字内容。"})]})]}),n.jsxs("footer",{children:[n.jsx("button",{autoFocus:!0,className:"thread-delete-dialog-cancel",onClick:t,type:"button",children:"取消"}),n.jsxs("button",{className:"thread-delete-dialog-confirm",onClick:o,type:"button",children:[n.jsx(ye,{size:15}),"确认删除"]})]})]})})}function Vr(e){const r=e.replace(/\s+/g," ").trim();return r.length>100?`${r.slice(0,100).trimEnd()}…`:r}function Jr(e){const r=t=>String(t).padStart(2,"0");return`${e.getFullYear()}-${r(e.getMonth()+1)}-${r(e.getDate())} ${r(e.getHours())}:${r(e.getMinutes())}:${r(e.getSeconds())}`}export{Wt as P,Fr as T,ta as a,_r as b,Br as c,ra as d,ea as f,Kt as w};
