...
浅色正文背景和灰色底栏参考 `annual/style.css`、`annual/background.js`，使用 5px 间隔的静态点阵与鼠标附近的柔和高亮，配色调整为灰绿色；已移除联系方式两侧的点列与图标。`assets/js/home-background.js` 每帧合并指针更新，按各区域坐标定位高亮；深绿区块和不透明内容卡片遮住装饰层，背景不拦截点击。触屏、减少动态效果和强制配色时不显示动态高亮；滚动、离开窗口和页面隐藏时收起高亮。底栏保留 `#bbbbbb` 底色和原赞助图片。
