...
独立入口为 `/annual/`，按年份倒序列出 `annual/年份/index.html`。从目录进入时，加载页先完整下载并校验该年份目录的静态文件，再打开原始年刊。各年刊仍是可单独运行的完整静态网站，页面结构、样式、脚本、资源和导航由年刊自己决定，目录不注入任何布局或代码。文件结构、加载范围和档案室收录方法见 [annual/README.md](annual/README.md)。
