...
目前包含 `2026/`（绿色山野）和 `2025/`（暖色海岸）两份简单 demo，页面标有 DEMO，文字为示例内容。两份 demo 各自包含 `index.html`、`style.css`、`script.js` 和 `assets/cover.svg`，支持展开、收起札记，可用于查看目录排序和独立页面效果。正式年刊可直接替换对应年份内的文件。
