...
此目录是独立的静态首页方案，不替换根目录 `index.php`，不修改 API、论坛入口或数据库。无需安装依赖或构建；页面与图片均使用本地资源。包括响应式导航、手动切换照片、原生可展开活动介绍、远征历史、游记和视频入口。禁用 JavaScript 时仍能阅读页面、展开活动内容和使用导航。视频和网盘沿用原首页的外部链接。
