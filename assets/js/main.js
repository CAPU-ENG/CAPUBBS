...
function addimg() {
    ...
    if (lst == 7) {
        $('#img_error').html("图片数已到上限");
        return;
    }

    $('#img_error').hide();
    var x = inputimg;
    if (x.length > 25) x = x.substr(0, 25) + "....";
    var y = inputimgthumb;
    if (y.length > 25) y = y.substr(0, 25) + "....";
...
function check_valid() {
    ...
    */
    ...
    if (inputimg == "") {
        img_error.html("<strong>图片地址不能为空</strong>");
        $('#inputimg').focus();
        return;
    }
    if (inputimgthumb == "") {
        img_error.html("<strong>缩略图地址不能为空</strong>");
        $('#inputimgthumb').focus();
        return;
    }
    var state = 0;
    ...
    */
    ...
    img2.onload = function () {
        ...
        width = img2.width;
        height = img2.height;
        if (width <= 310 || height / width < 0.0001 || height / width > 10000) {
            valid2 = -1;
            state = -1;
            img_error.text("缩略图长宽不小于310且宽高比应在1:10000和10000:1之间，当前" + width + "*" + height + " (" + (height / width) + ")");
            $("#inputimgthumb").focus();
            return;
        }
        valid2 = 1;
        if (valid1 == 1 && valid3 == 1) {
            state = -1;
            addimg();
        }
    }
