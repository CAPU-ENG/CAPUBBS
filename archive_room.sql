...
-- relative to CAPUBBS_ARCHIVE_ROOT in archive_entries.relative_path. Post
-- entries contain only target_url metadata and have no relative path.
-- entry_key is generated by PHP, not by MySQL:
--   file:   HMAC-SHA-256(upload timestamp + ':' + SHA-256(file bytes))
--   folder/post: HMAC-SHA-256(create timestamp + ':' + random value)
--
